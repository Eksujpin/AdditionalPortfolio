...
module internal StateMonad
    ...
    let characterValue (pos : int) : SM<char> = 
        S (fun s -> if s.word.Length >= (pos+1) then Success (fst(s.word.Item pos), s) else Failure (IndexOutOfBounds pos))

    let pointValue (pos : int) : SM<int> =
        S (fun s -> if s.word.Length >= (pos+1) then Success (snd(s.word.Item pos), s) else Failure (IndexOutOfBounds pos))      
    ...
    let declare (var : string) : SM<unit> = 
        S (fun s -> 
            match s with
            | r when s.reserved.Contains var -> Failure(ReservedName var)
            | e -> 
                match s.vars with
                ...
                | m :: ms ->
                    ...
                    | None -> Success ((), {s with vars = (Map.empty.Add (var,0)) :: s.vars})
            )
