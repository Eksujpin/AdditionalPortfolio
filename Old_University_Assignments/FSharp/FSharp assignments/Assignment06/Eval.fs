module Eval
    ...
    let rec stmntEval stmnt : SM<unit> = 
        ...
        | While(guard, stm)         -> boolEval guard >>= (fun x -> if x then push >>>= stmntEval stm >>>= pop >>>= stmntEval (While (guard,stm)) else ret ())
                                                                   //we didnt evaluate the body ^^ so therefor the loop was overflowing 
...
(* Part 4 *) 
    ...
    type boardFun = coord -> Result<squareFun option, Error> 
                                        ...
                                        // v this wasent a touple so it broke = was fun x y -> shoud be fun (x,y) 
    let stmntToBoardFun stm squares = fun (x,y) -> 
        stmntEval stm >>>= lookup "_result_" >>= (fun whatever -> 
        ret (Map.tryFind whatever squares)) |> 
        // right ide to map.tryFind on result of lookup + sqares (was m) but we needed to return it aka put ret ( *result*)
