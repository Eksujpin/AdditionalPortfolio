...
type expr = 
  ...
  | Let of (string * expr) list * expr // <-- exe 2.1
...
let rec freevars e : string list =
    ...
    | Let ((x,erhs) :: y, ebody) ->
      union (freevars erhs, minus (freevars Let (y,ebody), [x])) // <-- exe 2.2 
...
let rec tcomp (e : expr) (cenv : string list) : texpr =
    ...
    | Let ((x, erhs) :: y, ebody) ->    // <-- exe 2.3
      let cenv1 = x:: cenv
      TLet(tcomp erhs cenv, tcomp Let(y,ebody) cenv1)
