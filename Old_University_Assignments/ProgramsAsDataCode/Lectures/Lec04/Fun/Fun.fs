...
type value = 
  ...
  // Closure changed for Exercise 4.3
...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Letfun(f, xs, fBody, letBody) ->              // Exercise 4.3 (no functionality change)
    ...
    | Call(Var f, eArg) -> 
      ...
      | Closure (f, xs, fBody, fDeclEnv) ->         
        let xVals = List.map (fun x -> Int(eval x env)) eArg           // Exercise 4.3
        let fBodyEnv = (List.zip xs xVals) @ (f, fClosure) :: fDeclEnv // Exercise 4.3
        eval fBody fBodyEnv
...
let run e = eval e [];;
(* Comment out all examples, since they are not compatible with 4.3 rules
