...
void sweepPhase() {
  ...
  word* heapPointer;
  heapPointer = heap;
  while (heapPointer < afterHeap)
  {
      if (Color(*heapPointer) == Black) {
          *heapPointer = Paint(*heapPointer, White);
      }
      else if (Color(*heapPointer) == White) {
          *heapPointer = Paint(*heapPointer, Blue);
          heapPointer[1] = (word*)freelist;
          freelist = heapPointer;
      }
      heapPointer = heapPointer + Length(heapPointer[0]) + 1;
  }
}
